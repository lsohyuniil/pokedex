...
const PokemonDetail = () => {
  ...
  if (!pokemonData) return <div className="loading">잠시만 기다려주세요!</div>;
  ...
  return (
    <div className="box">
      <div className="page-btns">
        <Link
          className="prev-btn"
          to={`/pokemon/${pokemonId === 1 ? 1025 : pokemonId - 1}`}
        >
          &lt; No.{pokemonId === 1 ? 1025 : pokemonId - 1}{" "}
          {pokemonData.이전포켓몬이름}
        </Link>
        <Link
          className="next-btn"
          to={`/pokemon/${pokemonId === 1025 ? 1 : pokemonId + 1}`}
        >
          No.{pokemonId === 1025 ? 1 : pokemonId + 1}{" "}
          {pokemonData.다음포켓몬이름} &gt;
        </Link>
      </div>
        ...
        <div className="bottom-box">
          ...
          <div className="pokemon-description">
            ...
            <div className="pokemon-description-detail">
              <p>
                <strong>분류:</strong> {pokemonData.분류}
              </p>
              <p>
                <strong> / 특성:</strong> {pokemonData.특성.join(", ")}
              </p>
              <p>
                <strong> / 폼:</strong> {pokemonData.폼}
              </p>
              <p>
                <strong> / 키:</strong> {pokemonData.키}
              </p>
              <p>
                <strong> / 몸무게:</strong> {pokemonData.몸무게}
              </p>
            </div>
          </div>
          {/* 포켓몬 진화 과정 */}
