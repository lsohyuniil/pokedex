...
export const fetchPokemonDatas = async (id) => {
  try {
    ...
    const prevName = prevNameData.names.find(
      (name) => name.language.name === "ko"
    )?.name;
    ...
    const nextName = nextNameData.names.find(
      (name) => name.language.name === "ko"
    )?.name;

    // 한글 분류
    const koreanGenus = speciesData.genera.find(
    ...
    // 포켓몬 능력
    const translatedAbilities = pokemonData.abilities.map(
      (a) => a.ability.name
    );
  ...
  try {
    ...
    const getEvolutionChain = (chain) => {
      if (chain.species && chain.species.url) {
        ...
        if (lastNumber) evolutionIds.push(lastNumber);
      }
    ...
    return detailedData.filter((pokemon) => pokemon !== null);
  } catch (error) {
  ...
  try {
    ...
    for (let i = 1; i < results.length; i++) {
      filteredPokemon = filteredPokemon.filter(
        (p) => results[i].some((p2) => p2.name === p.name) // 같은 이름을 가진 포켓몬만 남김
      );
    }
    ...
    const detailedPokemonList = await Promise.all(
      filteredPokemon.map(async (pokemon) => {
        try {
          ...
          return fetchPokemonData(details.id);
        } catch (e) {
    ...
    console.error(
      `Error fetching Pokémon by types (${selectedTypes.join(", ")}):`,
      error
    );
    return [];
  }
